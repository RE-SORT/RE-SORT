...
from re_sortctr.pytorch.layers import FeatureEmbedding, MLP_Block, ScaledDotProductAttention, LogisticRegression

class Baseline_SA_DoubleStream(BaseModel):  
    def __init__(self, 
                 feature_map, 
                 model_id="Baseline_SA_DoubleStream",  
                 gpu=-1, 
                 learning_rate=1e-3, 
                 embedding_dim=10, 
                 ...
                 **kwargs):
        super(Baseline_SA_DoubleStream, self).__init__(feature_map, 
                                                      model_id=model_id, 
                                                      gpu=gpu, 
        ...
        self.self_attention_stream1 = nn.Sequential(
            *[MultiHeadSelfAttention(embedding_dim if i == 0 else attention_dim,
             ...
             for i in range(2)])  
        self.self_attention_stream2 = nn.Sequential(
            *[MultiHeadSelfAttention(embedding_dim if i == 0 else attention_dim,
                                     attention_dim=attention_dim, 
                                     num_heads=num_heads, 
                                     dropout_rate=net_dropout, 
                                     ...
                                     layer_norm=layer_norm) \
             for i in range(2)]) 
        self.fc = nn.Linear(2 * feature_map.num_fields * attention_dim, 1) 
    ...
    def forward(self, inputs):
        ...
        feature_emb_stream1 = self.embedding_layer_stream1(X)
        feature_emb_stream2 = self.embedding_layer_stream2(X)  
        attention_out_stream1 = self.self_attention_stream1(feature_emb_stream1)
        attention_out_stream2 = self.self_attention_stream2(feature_emb_stream2)
        
        attention_out_stream1 = torch.flatten(attention_out_stream1, start_dim=1)
        attention_out_stream2 = torch.flatten(attention_out_stream2, start_dim=1)
        attention_out = torch.cat([attention_out_stream1, attention_out_stream2], dim=1)
        
        y_pred = self.fc(attention_out)
        
        if self.dnn is not None:
            y_pred += self.dnn(feature_emb_stream1.flatten(start_dim=1))
            y_pred += self.dnn(feature_emb_stream2.flatten(start_dim=1))
